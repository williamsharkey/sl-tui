...
export class ObjectStoreFull extends ObjectStoreLite implements IObjectStore
{
    ...
    protected objectUpdateTerse(objectUpdateTerse: ImprovedTerseObjectUpdateMessage): void
    {
        ...
        for(const objectData of objectUpdateTerse.ObjectData)
        {
            ...
            // In StoreMyAttachmentsOnly mode, only process terse updates for self avatar and known objects
            if ((this.options & BotOptionFlags.StoreMyAttachmentsOnly) && localID !== this.agent.localID)
            {
                const o = this.objects.get(localID);
                if (!o) continue;
                // Still process avatars
                if (o.PCode !== PCode.Avatar) continue;
            }
